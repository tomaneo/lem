...
(defun replace-state-self-insert-hook-function (char)
  (declare (ignore char))
...
(defmethod state-enabled-hook ((state replace-state))
  (state-enabled-hook (ensure-state 'insert)))

(defmethod state-disabled-hook ((state replace-state))
  (state-disabled-hook (ensure-state 'insert)))
